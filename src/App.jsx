...
import { AlertCircle, Package, ShoppingCart,  LogOut, Menu, X, LayoutList, ChefHat, Utensils, DollarSign, Store, Factory, ShoppingBag, Wallet, Settings, BookUser } from 'lucide-react';
...
const usePermisos = () => {
  ...
  // Crear un Set con los strings de permisos para búsqueda O(1)
  const permisosSet = useMemo(() => 
    new Set(permisos?.map(p => p.permisos) || []), 
    [permisos]
  );
  
  const tienePermiso = (modulo, accion = 'ver') => {
    return permisosSet.has(`${modulo}.${accion}`) ||
           permisosSet.has(`${modulo}.*`) ||
           permisosSet.has('*.*');
  };
...
export { usePermisos }; // ✅ Exportar para usar en otros archivos
  ...
  React.useEffect(() => {
    if (token) {
      try {
        ...
        //console.log(payload.data.permisos);
        setUser(payload.data);
        setPermisos(payload.data.permisos);
      } catch (error) {
...
const MainLayout = ({ children }) => {
  ...
  const todosLosMenuItems  = [
    ...
    { id: 'usuarios', nombre: 'User | Rol | Permisos', icon: BookUser, color: 'red' }
  ];

  // ✅ Filtrar menú según permisos
  const menuItems = todosLosMenuItems.filter(item => tienePermiso(item.id));
  
  // ✅ Establecer módulo inicial al primer permitido
  useEffect(() => {
    if (!moduloActivo && menuItems.length > 0) {
      setModuloActivo(menuItems[0].id);
    }
  }, [menuItems, moduloActivo]);
...
const ModuloRouter = ({ moduloActivo }) => {
  const { token, user, permisos } = useAuth();
  const { tienePermiso } = usePermisos();

  // ✅ Validar permiso antes de renderizar
  if (!tienePermiso(moduloActivo)) {
    return (
  ...
  switch (moduloActivo) {
    ...
    case 'usuarios':
      return <ModuloUserRolPermisos token={token} />;
  }
};
